...
public:
    ...
    void CopyBufferContent(vkt::Context &context,
                           VkCommandBuffer cmdBuffer,
                           VkBuffer src, VkBuffer dst,
                           VkBufferCopy bufferCopy) const
    {
        const DeviceInterface &deviceInterface = context.getDeviceInterface();
        const VkDevice deviceVk                = context.getDevice();
    ...
    de::MovePtr<BufferWithMemory> copyDeviceBufferToHost(vkt::Context &context, BufferWithMemory &buffer) const
    {
        ...
        const auto resultBufferCreateInfo = makeBufferCreateInfo(
            resultBufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
...
public:
    ...
    bool verifyResultChunk(const void *inBufferPtr, uint32_t size, uint32_t offset) const
    {
        ...
        for (uint32_t nRay = 0; nRay < size; ++nRay)
        {
            ...
            const HitProperties *hitPropsPtr =
                reinterpret_cast<const HitProperties *>(resultU32Ptr) + nRay;
    ...
    bool verifyResultBuffer(vkt::Context &context, BufferWithMemory &buffer) const final
    {
        ...
        de::MovePtr<BufferWithMemory> headerBufferPtr =
        ...
        uint32_t chunkSize = itemsInChunk * sizeof(uint32_t);
        ...
        de::MovePtr<BufferWithMemory> chunkBufferPtr =
            de::MovePtr<BufferWithMemory>(new BufferWithMemory(
        ...
        for (uint32_t chunk = 0; chunk < amount; ++chunk)
        {
            ...
            if (!verifyResultChunk(chunkDataPtr, itemsInChunk, srcOffset / sizeof(uint32_t)))
            {
                return false;
            }
	}
...
de::MovePtr<BufferWithMemory> RayTracingMiscTestInstance::runTest(void)
{
    ...
    beginCommandBuffer(deviceInterface, *cmdBufferPtr, 0u /* flags */);
    {
        ...
        // Destroy everything if startdata was zero ...
        if (m_testPtr->getResultBufferStartData().size() == 0)
        {
            deviceInterface.cmdFillBuffer(*cmdBufferPtr, **gpuBufferPtr, 0, /* dstOffset */
            ...
            const VkBufferCopy bufferCopy{ 0, 0, resultBufferSize };
            deviceInterface.cmdCopyBuffer(*cmdBufferPtr, **startBufferPtr, **gpuBufferPtr, 1, &bufferCopy);
        }

        {
            ...
            const auto descriptorResultBufferInfo = makeDescriptorBufferInfo(**gpuBufferPtr, 0, /* offset */
                                                                             resultBufferSize);
